...
*/
       if(DataContainer::GetData().Directions[0] == DataContainer::Up){
            if(DataContainer::GetData().Positions[0].y -1 < 0)
           {
               DataContainer::GetData().Positions.push_front({DataContainer::GetData().Positions[0].x,static_cast<uint8_t>(DataContainer::GetData().Rows -1)});
            }else
            {
            DataContainer::GetData().Positions.push_front({DataContainer::GetData().Positions[0].x,static_cast<uint8_t>(DataContainer::GetData().Positions[0].y -1)});
            }
       }else if(DataContainer::GetData().Directions[0] == DataContainer::Down){
           if(DataContainer::GetData().Positions[0].y + 1 >= DataContainer::GetData().Rows){
               DataContainer::GetData().Positions.push_front({DataContainer::GetData().Positions[0].x , 0});
           }else{
         DataContainer::GetData().Positions.push_front({DataContainer::GetData().Positions[0].x,static_cast<uint8_t>(DataContainer::GetData().Positions[0].y +1)});
           }}else if(DataContainer::GetData().Directions[0] == DataContainer::Right){
           if(DataContainer::GetData().Positions[0].x +1 >= DataContainer::GetData().Columns){
               DataContainer::GetData().Positions.push_front({0,DataContainer::GetData().Positions[0].y });
           }else{
           DataContainer::GetData().Positions.push_front({static_cast<uint8_t>(DataContainer::GetData().Positions[0].x +1),DataContainer::GetData().Positions[0].y });
           } }else if(DataContainer::GetData().Directions[0] == DataContainer::Left){
           if(DataContainer::GetData().Positions[0].x -1 <0 ){
               DataContainer::GetData().Positions.push_front({static_cast<uint8_t>(DataContainer::GetData().Columns-1),DataContainer::GetData().Positions[0].y });
           }else{  DataContainer::GetData().Positions.push_front({static_cast<uint8_t>(DataContainer::GetData().Positions[0].x -1),DataContainer::GetData().Positions[0].y });
           }}
       DataContainer::GetData().Positions.pop_back();
            UpdateTileStates();
