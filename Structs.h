...
struct Position{
    Q_GADGET
public:
    uint8_t x,y,Displacement;
    Q_PROPERTY(uint8_t Displacement READ getDisplacement)
    ...
    uint8_t getDisplacement(){ return Displacement;}
    Position(uint8_t xval = 0, uint8_t yval = 0,uint8_t Disval = 0): x(xval),y(yval),Displacement(Disval){}
};
#endif // STRUCTS_H
